...
backgroundImg.src = './class.jpg'; // Đảm bảo đường dẫn đúng

function heart(x, y, r, a) {
    // Tải hình ảnh
    var img = new Image();
    img.src = './class.jpg'; // Đường dẫn đến ảnh nền

    img.onload = function () {
        // Điều chỉnh kích thước ảnh theo bán kính r
        var size = r * 2; // Kích thước ảnh dựa trên bán kính
        ctx.save();
        ctx.translate(x, y); // Di chuyển đến vị trí cần vẽ
        ctx.rotate(a * rad); // Xoay ảnh nếu cần
        ctx.drawImage(img, -size / 2, -size / 2, size, size); // Vẽ ảnh ở trung tâm
        ctx.restore();
    };
}
